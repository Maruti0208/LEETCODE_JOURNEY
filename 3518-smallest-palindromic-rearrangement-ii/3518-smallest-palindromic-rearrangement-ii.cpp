...
public:
    ...
    static inline int perm(const array<int, 26>& freq, unsigned seen, int sz) {
        int64_t ans=1;
        for ( ; seen; seen&=(seen-1)) {
            const int c=__builtin_ctz(seen);
            const int f=freq[c];
            ans*=comb(sz, f);
            if (ans>=INF) return INF;
            sz-=f;
